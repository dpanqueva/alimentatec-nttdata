...
import com.nttdata.ofertas.common.model.dto.response.ResponseDTO;
...
public class ExceptionsHandler {
    ...
    public final ResponseEntity<ResponseDTO<Object>> exceptionsClientInternalServerError(Exception e) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ResponseDTO<>(response(), EResponseType.ERROR, "Internal server error by ofertas", "500"));
    }
    ...
    public final ResponseEntity<ResponseDTO<Object>> exceptionsClientBadRequest(Exception e) {
        log.info(e.getClass().getName());
        log.error(e.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseDTO<>(response(), EResponseType.ERROR, "Bad Request by ofertas", "400"));
    }

    private OffersDTO response() {
        return null;
    }
}
